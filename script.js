document.addEventListener('DOMContentLoaded', () => {
    ...
    class Entidade {
        constructor(x, y, classeCss) {
            ...
            this.x = x ?? Math.random() * (world.offsetWidth - 50);
            this.y = y ?? Math.random() * (world.offsetHeight - 50);
    ...
    class Animal extends Entidade {
        ...
        decidirAcao() {
            ...
            if (this.isMature && this.gender === 'female' && this.reproductionUrge > 70 && this.fome < 40 && this.sede < 40) {
                this.alvo = this.encontrarMaisProximo(animais.filter(a => a.tipo === this.tipo && a.isMature && a.gender === 'male'));
                this.estado = 'buscandoParceiro'; return;
            }
        ...
        interagirComAlvo() {
            switch(this.estado) {
                case 'buscandoComida':
                    ...
                    this.lastKnownFoodLocation = { x: this.alvo.x, y: this.alvo.y };
                    this.alvo.remover();
        ...
        darALuz() {
            ...
            for(let i=0; i<numFilhotes; i++) {
                animais.push(new Animal(this.tipo, this.x, this.y));
            }
        }
        morrer() {
            ...
            // O corpo fica no cenário, mas o objeto é removido da lista de ativos
        }
    }
    ...
    function popularSetup() {
        ...
        for (const [tipo, def] of Object.entries(DEFINICOES_ANIMAIS)) {
            ...
            group.innerHTML = `
                ...
                <input type="number" id="num-${tipo}" data-tipo="${tipo}" value="0" min="0" max="20">
            `;
    ...
    function iniciar() {
        ...
        // Comidas (distribui entre os tipos de animais selecionados)
        const tiposAnimaisSelecionados = Array.from(animalSelectionDiv.querySelectorAll('input'))
            .filter(input => input.value > 0).map(input => input.dataset.tipo);
        
        let comidaCount = 0;
        while(comidaCount < numComidaInput.value) {
    ...
    function gameLoop() {
        ...
        animalCountsDiv.innerHTML = Object.entries(counts)
            .map(([tipo, count]) => `<span>${DEFINICOES_ANIMAIS[tipo].nome}: ${count}</span>`)
            .join('<br>');
